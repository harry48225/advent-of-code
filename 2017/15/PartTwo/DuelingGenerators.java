...
public class DuelingGenerators
{
	public static void main(String[] args)
	{
		...
		dG.partTwo(); // Call the part one method
	}
	
	private void partTwo()
	{
		Generator a = new Generator(16807, 883, 4); // Create a new generator for a
		Generator b = new Generator(48271, 879, 8); // Create a new generator for b
		int numberOfMatches = 0; // Store the number of matches
		
		for (int i = 0; i<5000000; i++) // Do this 5 million times
		{
	...
	private class Generator // Generator class
	{
		...
		private int requiredFactor = -1; 
		
		public Generator(int tempFactor, int tempStartingValue, int tempRequiredFactor) // Constructor
		{
			factor = tempFactor; // Set the factor
			previousValue = tempStartingValue; // Set the starting value
			requiredFactor = tempRequiredFactor; // Set the required factor
		}
		
		public String generateValue() // Method to generate a value
		{
			long newValue = -1; // Initalise the new value variable
			
			while ((newValue % requiredFactor) != 0) // While the required factor is not a factor
			{	// Generate values
				newValue = previousValue * factor; // Multiply by the factor
