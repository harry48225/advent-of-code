...
public class CoprocessorConflagrationPartTwo
{
	private final String FILE_NAME = "puzzleInputMoreOptimised.txt";
	...
	{
		CoprocessorConflagrationPartTwo cC = new CoprocessorConflagrationPartTwo(); // Create a new instance of the class
		cC.partTwo(); // Call the partTwo method
	}
	...
	private void partTwo() // For part two of the challenge
	{
		...
		registers.put("a", new Long(1)); // Set register a to 1
		...
		while (executing) // While we are executing
		{
			...
			else if (opcode.equals("jnz")) // If it's the jump instruction
			{
				...
				if (getOperandValue(operand1) != 0) // If the value of the register/number is greater than 0
				{
					...
					System.out.println("Jumped! " + jumpAmount);
				}
			}
			...
			// Check if d is prime
			// If so we need to subtract one from h
			if (getRegisterValue("d") > 0) // If d isn't zero
			{
				boolean prime = true;
				for (int i = 0; i < getRegisterValue("d"); i++)
				{
					if (getRegisterValue("d") % i == 0)
					{
						prime = false;
						break;
					}
				...
				if (prime) // If it's prime
				{
					...
					registers.put("h", getRegisterValue("h") - 1); // Subract one from h
				}
			}
			
		}
		System.out.println("h: " + getRegisterValue("h"));
